...
def save_current_profile():
    feelings = get_user_feelings()
    if name := entry_profile.get().strip():
...
def refresh_profile_list():
    ...
    for p in profiles:
        menu.add_command(
            label=p,
            command=lambda v=p: entry_profile.delete(0, tk.END)
            or entry_profile.insert(0, v),
        )
...
tk.Label(
    root,
    text="💠 T13.3 AI Central Interface",
    font=("Arial", 18, "bold"),
    fg="#00ffd1",
    bg="#1e1e1e",
).pack(pady=8)
...
for idx, label in enumerate(["Joy", "Stress", "Hope", "Fear", "Energy"]):
    tk.Label(frm_emotions, text=label, fg="white", bg="#1e1e1e").grid(
        row=0, column=idx, padx=5
    )
entry_joy = tk.Entry(frm_emotions, width=5)
entry_joy.grid(row=1, column=0)
entry_stress = tk.Entry(frm_emotions, width=5)
entry_stress.grid(row=1, column=1)
entry_hope = tk.Entry(frm_emotions, width=5)
entry_hope.grid(row=1, column=2)
entry_fear = tk.Entry(frm_emotions, width=5)
entry_fear.grid(row=1, column=3)
entry_energy = tk.Entry(frm_emotions, width=5)
...
entry_profile = tk.Entry(frm_profile, width=20)
entry_profile.grid(row=0, column=1, padx=5)
...
tk.Button(
    frm_profile, text="💾 Save", command=save_current_profile, bg="#444", fg="white"
).grid(row=0, column=3, padx=5)
tk.Button(
    frm_profile,
    text="📂 Load & Run",
    command=load_and_run_profile,
    bg="#555",
    fg="white",
).grid(row=0, column=4)
refresh_profile_list()
...
buttons = [("🧠 Personality", show_personality), ("🎯 Run Central Core", run_central)]
for txt, cmd in buttons:
    tk.Button(frm_buttons, text=txt, command=cmd, width=20, bg="#333", fg="white").pack(
        side="left", padx=5
    )

# Output area
output = scrolledtext.ScrolledText(
    root, bg="#252526", fg="#d4d4d4", font=("Consolas", 11)
)
