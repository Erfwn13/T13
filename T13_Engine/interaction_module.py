...
class TextInteraction:
    ...
    def respond(self, user_input):
        ...
        if is_persian(user_input) or lang == "fa":
            smart_response = generate_smart_response(user_input, self.memory, self.style)
            if smart_response and not smart_response.startswith("متأسفم دقیقاً نفهمیدم"):
                return smart_response
            # مدل عمیق فارسی
            deep_model_fa = DeepConversationalModel(model_name="auto", device="cuda" if torch.cuda.is_available() else "cpu", lang="fa")
            if hasattr(deep_model_fa, 'error') and ("HooshvareLab" in deep_model_fa.error or "not found" in deep_model_fa.error or "404" in deep_model_fa.error):
                return ("مدل GPT2 فارسی نصب نیست. برای نصب مدل فارسی این دستور را اجرا کن:\n"
            ...
            smart_response = generate_smart_response_en(user_input, self.memory, self.style)
            if smart_response and not smart_response.startswith("Sorry, I didn't get that"):
                return smart_response
            # مدل عمیق انگلیسی
            deep_model_en = DeepConversationalModel(model_name="auto", device="cuda" if torch.cuda.is_available() else "cpu", lang="en")
