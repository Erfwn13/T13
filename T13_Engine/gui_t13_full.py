...
from profile_manager import list_profiles, load_profile, save_profile
...
# --- GUI actions ---
def run_custom_all():
    run_with_output(lambda: ai.run_all(get_user_feelings(), options))


def run_memory():
    run_with_output(ai.run_memory)


def run_emotion():
    run_with_output(lambda: ai.run_emotion(get_user_feelings()))


def run_decision():
    run_with_output(lambda: ai.run_decision(options))


def run_behavior():
    run_with_output(ai.run_behavior)


def run_upgrade():
    run_with_output(ai.run_upgrade)


def run_health():
    run_with_output(ai.run_health)


def run_trend():
    run_with_output(ai.run_trend_analysis)


def run_multiverse():
    run_with_output(ai.run_multiverse)
...
def run_central():
    threading.Thread(target=lambda: run_with_output(central.run_all)).start()
...
def save_current_profile():
    feelings = get_user_feelings()
    if name := profile_name.get().strip():
...
def refresh_profile_menu():
    ...
    for p in profiles:
        menu.add_command(
            label=p,
            command=lambda v=p: profile_name.delete(0, tk.END)
            or profile_name.insert(0, v),
        )
...
tk.Label(
    root,
    text="💠 T13.3 Full Central Interface",
...
entry_joy = tk.Entry(form, width=5)
entry_joy.grid(row=1, column=0)
entry_stress = tk.Entry(form, width=5)
entry_stress.grid(row=1, column=1)
entry_hope = tk.Entry(form, width=5)
entry_hope.grid(row=1, column=2)
entry_fear = tk.Entry(form, width=5)
entry_fear.grid(row=1, column=3)
entry_energy = tk.Entry(form, width=5)
entry_energy.grid(row=1, column=4)

tk.Button(
    root,
    text="🚀 Run with My Emotions",
...
profile_name = tk.Entry(profile, width=25)
...
tk.Button(
    profile, text="💾 Save", command=save_current_profile, bg="#444", fg="white"
).grid(row=0, column=3)
tk.Button(
    profile, text="📂 Load & Run", command=load_and_run_profile, bg="#555", fg="white"
).grid(row=0, column=4)
refresh_profile_menu()
...
output = scrolledtext.ScrolledText(
    root, font=("Consolas", 11), bg="#000", fg="#0f0", insertbackground="white"
)
output.pack(expand=True, fill="both", padx=10, pady=10)

# Status bar
status_bar = tk.Label(
    root, text="", anchor="w", font=("Segoe UI", 10), bg="#1e1e1e", fg="#aaaaaa", pady=4
)
